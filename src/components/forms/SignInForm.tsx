    ...
    return (
        <div className='flex flex-col items-center justify-center w-[40%] gap-lg_gap'>
            <div className='flex flex-col items-center justify-center gap-lg_gap'>
                <Logo />
                <div className='text-md_text'>Авторизація</div>
            </div>
            <div className='flex flex-col items-center justify-center w-full gap-md_gap'>
                <div className='relative h-[40px] w-full'>
                    <input
                        className='h-full w-full text-dark text-sm_text bg-transparent border border-1 border-solid border-passive rounded-sm_radius outline-none p-lg_p'
                        type='text'
                        value={signInData.username}
                        onChange={e => setSignInData({ ...signInData, username: e.target.value })}
                        required />
                    {/* <span className='absolute left-0 p-[10px] pointer-events-none text-sm_text text-passive'>Username</span> */}
                </div>
                <div className='relative h-[40px] w-full'>
                    <input
                        className='h-full w-full text-dark text-sm_text bg-transparent border border-1 border-solid border-passive rounded-sm_radius outline-none p-lg_p'
                        ...
                        required />
                    {/* <span className='absolute left-0 p-[10px] pointer-events-none text-sm_text text-passive'>Password</span> */}
                </div>
                <Button className='w-full text-light hover:bg-primary2 bg-primary' onClick={handleAuthRequest}>Увійти</Button>
            </div>
            {/* <div className='flex flex-col items-center justify-center text-sm_text text-passive gap-sm_gap'>
